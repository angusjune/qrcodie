...
`.github/workflows/publish-chrome.yml`. For example, manifest version `3.1.0`
must be released with tag `v3.1.0`.

Note that the older `v2.0` and `v3.0` tags predate this workflow and do not
follow that rule. New releases need the full three-part version, because the
workflow compares the tag against `manifest.json` exactly.
...
The workflow builds the extension with Yarn, saves the ZIP as a GitHub Actions
artifact, uploads it through Chrome Web Store API v2, and submits it with
...
**Reusing an existing setup.** If another extension already publishes from the
same Google Cloud project, reuse its project, service account, and workload
identity pool. A service account added to a Chrome Web Store publisher can
publish any item under that publisher, so steps 1 to 3 are already done. You
still need a provider and a binding scoped to *this* repository — attribute
conditions are per-repository, so an existing provider for another repository
will reject this one's tokens.
...
WIF_PROVIDER_ID="qrcodie-releases"
GITHUB_REPOSITORY="angusjune/qrcodie"
...
Create a workload identity pool. Skip this if you are reusing a pool that
already exists — pool IDs are unique per project, and recreating one fails:
...
Allow only workflows from `angusjune/qrcodie` to impersonate the Chrome Web
Store service account. Do not skip this command — without it every release
fails with `iam.serviceAccounts.getAccessToken` denied. Adding this binding
does not disturb bindings for other repositories on the same service account:
...
```text
projects/123456789/locations/global/workloadIdentityPools/github-actions/providers/qrcodie-releases
```
...
**4. Create the provider.** Open
[IAM & Admin → Workload Identity Federation][console-wif]. If the
`github-actions` pool already exists, open it and click **Add provider**;
otherwise start the **New workload provider and pool** flow and name the pool
`github-actions`.

On the provider step:

| Field | Value |
| --- | --- |
| Select a provider | `OpenID Connect (OIDC)` |
| Provider name | `QRCodie releases` |
| Provider ID | `qrcodie-releases` |
...
```text
assertion.repository == 'angusjune/qrcodie'
```
...
```text
projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/github-actions/providers/qrcodie-releases
```
...
`principalSet://` member ending in `/attribute.repository/angusjune/qrcodie`.
If the only member listed is another repository, step 5 above never took
effect for this one:
...
The provider must map `attribute.repository` and carry this repository's
...
```sh
gh api repos/angusjune/qrcodie/environments/chrome-web-store/variables \
...
| Variable | Value |
| --- | --- |
| `CWS_EXTENSION_ID` | `bphdkmejkbcfcglcgmmbmjggafkgfjcm` |
...
`manifest.json` is the authoritative version — it is what the workflow reads,
what the tag is compared against, and what ships to the store. Keep
`package.json` in step with it so the repository does not drift.

Check the build locally before tagging:
...
1. **The binding is missing.** Run the first check in
   [Verify the setup](#verify-the-setup). A policy that lists no member for
   this repository confirms it, and the `add-iam-policy-binding` command fixes
   it.
...
`yarn install --immutable` refuses to change `yarn.lock` in CI. Run
`yarn install` locally, commit the updated lockfile, and re-run. This usually
means a dependency was changed without committing the resulting lockfile.
...
```json
{
  "itemId": "bphdkmejkbcfcglcgmmbmjggafkgfjcm",
